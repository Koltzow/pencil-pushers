	...
	Test.create = function () {
		...
		for (var i = 0; i < 200; i++) {
			...
			(function () {
				...
				setTimeout(function () {
					...
					new EXP.ammo.energyball(boss.type, {x: boss.x + boss.width/2 + Math.cos(aimAngle)*32, y: boss.y + boss.height/2 + Math.sin(aimAngle)*32}, {x: Math.cos(aimAngle), y: Math.sin(aimAngle)});
		...
		boss.update = function () {
			...
			for (var i = 0; i < EXP.engine.bodies.length; i++) {
				...
				if( EXP.physics.collision.test(this, EXP.engine.bodies[i]) && EXP.engine.bodies[i].onCollisionEnter !== undefined ){
